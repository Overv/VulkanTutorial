...
// Destroy the Vulkan image associated with the texture.
// This releases the resources allocated for the image.
vkDestroyImage(gbl_04_device, textureImage_24, nullptr);

// Free the memory allocated for the texture image.
// This releases the memory resources associated with the image.
 ...
 *
 * This function loads an image from a file, creates a staging buffer to hold the image data,
 * and then creates a Vulkan image to store the texture. The image is then transitioned to the
 * appropriate layout for shader access.
 */
void createTextureImage_24() {
    ...
    //The buffer should be in host visible memory so that we can map it and
    ...
    void* data;
    // Map the staging buffer memory and copy the image data to it.
    vkMapMemory(gbl_04_device, stagingBufferMemory, 0, imageSize, 0, &data);
    memcpy(data, pixels, static_cast<size_t>(imageSize));
    vkUnmapMemory(gbl_04_device, stagingBufferMemory);

    // Free the image memory loaded by stb_image.
    stbi_image_free(pixels);

    // Create a Vulkan image to store the texture.
    ...
    // Transition the image layout to be optimal for data transfer.
...
/**
 * Creates a Vulkan ref_image.
 *
 * This function initializes a Vulkan ref_image with the specified parameters and allocates memory for it.
 *
 * @param width The width of the ref_image.
 * @param height The height of the ref_image.
 * @param format The format of the ref_image.
 * @param tiling The tiling arrangement of the ref_image.
 * @param usage The usage flags for the ref_image.
 * @param properties The memory properties for the ref_image.
 * @param ref_image A reference to the VkImage object to be created.
 * @param ref_imageMemory A reference to the VkDeviceMemory object to be allocated.
 */
void createImage_zcj_helper_of_createTextureImage_24(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& ref_image, VkDeviceMemory& ref_imageMemory) {
    ...
    if (vkCreateImage(gbl_04_device, &imageInfo, nullptr, &ref_image) != VK_SUCCESS) {
        throw std::runtime_error("failed to create ref_image!");
    }

    VkMemoryRequirements a_memRequirements_zcj_obtained;
    vkGetImageMemoryRequirements(gbl_04_device, ref_image, &a_memRequirements_zcj_obtained);

    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = a_memRequirements_zcj_obtained.size;
    allocInfo.memoryTypeIndex = findMemoryType(a_memRequirements_zcj_obtained.memoryTypeBits, properties);

    if (vkAllocateMemory(gbl_04_device, &allocInfo, nullptr, &ref_imageMemory) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate ref_image memory!");
    }

    vkBindImageMemory(gbl_04_device, ref_image, ref_imageMemory, 0);
}
    /**
 * Transitions the layout of a Vulkan image.
 *
 * This function records a command buffer to transition the layout of a Vulkan image
 * from an old layout to a new layout. It uses a pipeline barrier to synchronize the
 * transition.
 *
...
void transitionImageLayout_24(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        ...
        // Throw an exception if the layout transition is unsupported.
        throw std::invalid_argument("unsupported layout transition!");
    }

    // Insert a pipeline barrier to transition the image layout.
 ...
 *
 * This function records a command buffer to copy data from a buffer to a Vulkan image.
 * It uses a single-time command buffer for the copy operation.
 *
 * @param buffer The Vulkan buffer containing the data to copy.
 * @param image The Vulkan image to copy the data to.
...
void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    ...
    void createDescriptorSets_23() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            descriptorWrite.dstSet = gbl_23_descriptorSetS[i];
            ...
            vkUpdateDescriptorSets(gbl_04_device, 1, &descriptorWrite, 0, nullptr);
        }
    }
    ...
    VkCommandBuffer beginSingleTimeCommands_zcj_VckCommandBuffer_24() {
        ...
        //the command buffer will be submitted once and then reset.
