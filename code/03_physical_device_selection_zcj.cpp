...
struct QueueFamilyIndices_03 {
    std::optional<uint32_t> graphicsFamily_03; //A graphics queue family is not guaranteed, and the value may be empty if no suitable queue family is found.
...
/// \brief Global variable to store the selected physical device.
///
/// This variable is initialized to `VK_NULL_HANDLE` and will be set to a valid
/// `VkPhysicalDevice` handle once a suitable physical device is found.
