...
private:
    ...
    void createSwapChain_06() {
        SwapChainSupportDetails_06 a_swapChainSupport = querySwapChainSupport(gbl_03_physicalDevice);

        VkSurfaceFormatKHR a_surfaceFormat = chooseSwapSurfaceFormat_06(a_swapChainSupport.formats);
        VkPresentModeKHR a_presentMode = chooseSwapPresentMode(a_swapChainSupport.presentModes);
        VkExtent2D a_extent = chooseSwapExtent(a_swapChainSupport.capabilities);

        // Determine the number of images in the swap chain.
        // The minimum number of images is specified by minImageCount.
        // Adding 1 ensures there is at least one more image than the minimum required,
        // which can help improve performance by allowing the application to work on rendering
        // a new frame while the previous frame is still being displayed.
        uint32_t imageCount = a_swapChainSupport.capabilities.minImageCount + 1;

        if (a_swapChainSupport.capabilities.maxImageCount > 0 && imageCount > a_swapChainSupport.capabilities.maxImageCount) {
            imageCount = a_swapChainSupport.capabilities.maxImageCount;
        }
        ...
        createInfo.imageFormat = a_surfaceFormat.format;
        createInfo.imageColorSpace = a_surfaceFormat.colorSpace;
        createInfo.imageExtent = a_extent;
        // The `imageArrayLayers` field in the `VkSwapchainCreateInfoKHR` structure is typically set to 1 for standard 2D images.
        // However, there are scenarios where you might set it to a value greater than 1:
        // 1. **3D Images**: If you are working with 3D textures or images, you might need to set `imageArrayLayers` to the number of layers in the 3D image.
        // 2. **Array Textures**: If you are using array textures, which are essentially a collection of 2D textures, you would set `imageArrayLayers` to the number of layers in the array.
        // 3. **Stereo Rendering**: For applications that require stereo rendering (e.g., VR applications), you might set `imageArrayLayers` to 2 to handle the left and right eye images.
        // In these cases, you would adjust the `imageArrayLayers` field accordingly to match the requirements of your application.
        ...
        createInfo.preTransform = a_swapChainSupport.capabilities.currentTransform;
        createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
        createInfo.presentMode = a_presentMode;
        ...
        // Retrieve the number of images in the swap chain.
        // The vkGetSwapchainImagesKHR function is a Vulkan API call that retrieves the handles
        // of the images associated with the swap chain. The parameters passed to this function
        // include the logical device (device), the swap chain (gbl_06_swapChain), a pointer to a variable
        // that will store the number of images (imageCount), and a nullptr indicating that the function
        // should only query the number of images without actually retrieving them.
        vkGetSwapchainImagesKHR(gbl_04_device, gbl_06_swapChain, &imageCount, nullptr);(gbl_04_device, gbl_06_swapChain, &imageCount, nullptr);
...
//        gbl_06_swapChainImageFormat = a_surfaceFormat.format;
//        gbl_06_swapChainExtent = a_extent;
    }
