...
// Memory handle for the vertex buffer
...
// Free the memory allocated for the vertex buffer.
 ...
 * The vertex buffer is used to store vertex data for rendering.
 */
void createVertexBuffer_19() {
    VkBufferCreateInfo bufferInfo{};
    bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    bufferInfo.size = sizeof(vertices_19[0]) * vertices_19.size(); // Size of the buffer in bytes
    bufferInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; // Buffer usage as a vertex buffer
    bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // Buffer is exclusive to a single queue family

    // Create the vertex buffer
    if (vkCreateBuffer(gbl_04_device, &bufferInfo, nullptr, &vertexBuffer_19) != VK_SUCCESS) {
        throw std::runtime_error("failed to create vertex buffer!");
    }

    VkMemoryRequirements memRequirements;
    // Get memory requirements for the vertex buffer
    vkGetBufferMemoryRequirements(gbl_04_device, vertexBuffer_19, &memRequirements);

    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size; // Size of the memory allocation
    allocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT); // Memory type index

    // Allocate memory for the vertex buffer, i.e. vertexBufferMemory_19
    if (vkAllocateMemory(gbl_04_device, &allocInfo, nullptr, &vertexBufferMemory_19) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate vertex buffer memory!");
    }

    // Bind the allocated memory to the vertex buffer
    vkBindBufferMemory(gbl_04_device, vertexBuffer_19, vertexBufferMemory_19, 0);

    void* data;
    // Map the vertex buffer memory to a CPU accessible address
    vkMapMemory(gbl_04_device, vertexBufferMemory_19, 0, bufferInfo.size, 0, &data);
    // Copy vertex data to the mapped memory
    memcpy(data, vertices_19.data(), (size_t) bufferInfo.size);
    // Unmap the vertex buffer memory
    vkUnmapMemory(gbl_04_device, vertexBufferMemory_19);
}

/**
 * Finds a suitable memory type for a buffer.
 *
 * @param typeFilter A bitmask of the memory types that are suitable.
 * @param properties The properties required for the memory type.
...
uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    // Iterate over all memory types to find a suitable one
...
// Array of offsets for the vertex buffers
