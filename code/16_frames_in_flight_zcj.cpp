...
private:
    ...
    void initVulkan() {
        ...
        createCommandBuffer_S_15();  //cj-16
    ...
    void cleanup_00() {
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) { //cj-16
    ...
    void createCommandBuffer_S_15() {
        gbl_16_commandBufferS.resize(MAX_FRAMES_IN_FLIGHT);  //cj-16

        VkCommandBufferAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        allocInfo.commandPool = gbl_14_commandPool;
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        allocInfo.commandBufferCount = (uint32_t) gbl_16_commandBufferS.size();  //cj-16

        if (vkAllocateCommandBuffers(gbl_04_device, &allocInfo, gbl_16_commandBufferS.data()) != VK_SUCCESS) { //cj-16
    ...
    void drawFrame_15() {
        ...
        uint32_t imageIndex;
        vkAcquireNextImageKHR(gbl_04_device, gbl_06_swapChain, UINT64_MAX, gbl_16_imageAvailableSemaphoreS[gbl_16_currentFrame], VK_NULL_HANDLE, &imageIndex);
