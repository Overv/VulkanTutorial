...
#include <glm/glm.hpp>
#include <glm/gtc/matrix_transform.hpp> //22
...
/**
 * \brief Structure to hold transformation matrices for the vertex shader.
 *
 * This structure contains three transformation matrices: model, view, and projection.
 * These matrices are used to transform vertex positions from model space to screen space.
 */
struct UniformBufferObject {
    glm::mat4 model; ///< Model matrix: Transforms vertices from model space to world space.
    glm::mat4 view;  ///< View matrix: Transforms vertices from world space to camera space.
    glm::mat4 proj;  ///< Projection matrix: Projects vertices from camera space to screen space.
};
...
private:
    ...
    std::vector<void*> gbl_22_uniformBuffersMapped;  //CPU pointer to the uniform buffer memory
...
// Loop through each frame in flight and destroy the uniform buffer and free its memory
for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroyBuffer(gbl_04_device, gbl_22_uniformBuffers[i], nullptr); // Destroy the uniform buffer
    vkFreeMemory(gbl_04_device, gbl_22_uniformBuffersMemory[i], nullptr); // Free the memory allocated for the uniform buffer
}
...
/**
 * \brief Creates a descriptor set layout for the uniform buffer object.
 *
 * This function sets up a descriptor set layout binding for a uniform buffer object (UBO) and creates the descriptor set layout.
 * The descriptor set layout is used to define the interface between the application and the shaders.
 *
 * \throws std::runtime_error if the descriptor set layout creation fails.
 */
void createDescriptorSetLayout_22() {
    VkDescriptorSetLayoutBinding uboLayoutBinding{};
    uboLayoutBinding.binding = 0; ///< Binding index for the UBO.
    uboLayoutBinding.descriptorCount = 1; ///< Number of descriptors in the binding.
    uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; ///< Type of descriptor.
    uboLayoutBinding.pImmutableSamplers = nullptr; ///< Immutable samplers (not used for UBO).
    uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; ///< Shader stage that will access the UBO.

    VkDescriptorSetLayoutCreateInfo layoutInfo{};
    layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO; ///< Type of the structure.
    ...
    if (vkCreateDescriptorSetLayout(gbl_04_device, &layoutInfo, nullptr, &gbl_22_descriptorSetLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create descriptor set layout!"); ///< Error handling if creation fails.
    }
}
...
// Set the number of descriptor set layouts in the pipeline layout
pipelineLayoutInfo.setLayoutCount = 1;

// Pointer to the array of descriptor set layouts
 ...
 *
 * This function allocates and maps uniform buffers for each frame in flight.
 * The uniform buffers are used to store transformation matrices that are updated each frame.
 */
void createUniformBuffers_22() {
    VkDeviceSize bufferSize = sizeof(UniformBufferObject); ///< Size of the uniform buffer object.

    gbl_22_uniformBuffers.resize(MAX_FRAMES_IN_FLIGHT); ///< Resize the vector to hold buffers for each frame.
    gbl_22_uniformBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT); ///< Resize the vector to hold memory for each buffer.
    gbl_22_uniformBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT); ///< Resize the vector to hold mapped memory pointers.

    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        // Create a buffer for the uniform buffer object
        ...
        // Map the GPU buffer memory to a CPU pointer
 ...
 *
 * This function updates the uniform buffer object (UBO) with the transformation matrices
 * for the current frame. It calculates the model, view, and projection matrices and copies
 * them to the mapped uniform buffer memory.
 *
 * \param currentImage The index of the current frame.
 */
void updateUniformBuffer(uint32_t currentImage) {
    static auto startTime = std::chrono::high_resolution_clock::now(); ///< Start time for the animation.

    auto currentTime = std::chrono::high_resolution_clock::now(); ///< Current time for the animation.
    float time = std::chrono::duration<float, std::chrono::seconds::period>(currentTime - startTime).count(); ///< Elapsed time in seconds.

    UniformBufferObject ubo{};
    ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f)); ///< Model matrix: Rotates the model over time.
    ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)); ///< View matrix: Camera positioned at (2, 2, 2) looking at the origin.
    ubo.proj = glm::perspective(glm::radians(45.0f), gbl_06_swapChainExtent.width / (float) gbl_06_swapChainExtent.height, 0.1f, 10.0f); ///< Projection matrix: Perspective projection.
    ubo.proj[1][1] *= -1; ///< Invert the Y-axis for Vulkan's coordinate system.

    memcpy(gbl_22_uniformBuffersMapped[currentImage], &ubo, sizeof(ubo)); ///< Copy the UBO data to the mapped uniform buffer memory.
}
