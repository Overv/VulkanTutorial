...
private:
    ...
    /// \brief Flag indicating if the framebuffer has been resized.
    ///
    /// This global variable is set to true when the framebuffer is resized,
    /// and is used to trigger the recreation of the swap chain.
    ...
    void initWindow() {
        ...
        // Set the user pointer for the window to the current instance of the application
         ...
         *
         * This function is called whenever the framebuffer is resized. It sets the
         * framebufferResized flag to true, indicating that the swap chain needs to be
         * recreated.
         *
    ...
    /**
     * @brief Cleans up the swap chain by destroying framebuffers, image views, and the swap chain.
     *
     * This function iterates through the swap chain framebuffers and image views,
     * destroying each one. Finally, it destroys the swap chain itself.
     */
    void cleanupSwapChain_17() {
        // Destroy all framebuffers in the swap chain
        for (auto framebuffer : gbl_12_swapChainFramebuffers) {
            vkDestroyFramebuffer(gbl_04_device, framebuffer, nullptr);
        }

        // Destroy all image views in the swap chain
 ...
 *
 * This function performs cleanup of Vulkan resources by first cleaning up the swap chain,
 * then destroying the graphics pipeline, pipeline layout, and render pass.
 */
    void cleanup_00() {
     ...
     *
     * This function handles the recreation of the swap chain when the framebuffer is resized or becomes suboptimal.
     * It waits for the device to be idle, cleans up the existing swap chain, and then creates a new swap chain,
    ...
    void drawFrame_15() {
        ...
        // Check if the swap chain is out of date and needs to be recreated
        if (result == VK_ERROR_OUT_OF_DATE_KHR) {
            recreateSwapChain_17();
            return;
        } else if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
            // Throw an error if acquiring the image failed for any other reason
        ...
        // Check if the swap chain is out of date, suboptimal, or if the framebuffer was resized
        if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || gbl_17_framebufferResized) {
            ...
            // Throw an error if presenting the image failed for any other reason
