 ...
 *
 * This pool is created with the VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT flag,
 * allowing individual command buffers to be reset.
 */
VkCommandPool gbl_14_commandPool;

/**
 * Primary command buffer used for recording rendering commands.
 *
 * This command buffer is allocated from the command pool and is used to record
 * commands for rendering operations.
 */
 ...
 *
 * This function initializes a command pool with the VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT flag,
 * allowing individual command buffers to be reset.
 */
createCommandPool_14();

/**
 * Allocates a primary command buffer from the command pool.
 *
 * This function allocates a single primary command buffer from the previously created command pool.
 * The command buffer is used for recording rendering commands.
 */
 ...
 * Creates a command pool for allocating command buffers.
 *
 * This function initializes a command pool with the VK\_COMMAND\_POOL\_CREATE\_RESET\_COMMAND\_BUFFER\_BIT flag,
 * allowing individual command buffers to be reset. The command pool is associated with the graphics queue family.
 *
 * @throws std::runtime\_error if the command pool creation fails.
 */
 ...
 * Allocates a primary command buffer from the command pool.
 *
 * This function initializes a VkCommandBufferAllocateInfo structure with the command pool,
 * command buffer level, and the number of command buffers to allocate. It then calls
 * vkAllocateCommandBuffers to allocate the command buffer. If the allocation fails,
 * it throws a runtime error.
 *
 * @throws std::runtime_error if the command buffer allocation fails.
 */
void createCommandBuffer_14() {
 ...
 /**
 * Records commands into the given command buffer for rendering a frame.
 *
 * This function begins recording commands into the specified command buffer, sets up the render pass,
 * binds the graphics pipeline, sets the viewport and scissor, and issues a draw command.
 *
 ...
 *
 * @throws std::runtime_error if the command buffer recording fails.
 */
