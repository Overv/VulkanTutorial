...
private:
    ...
    void createGraphicsPipeline_08() {
        ...
        auto a_vertShaderCode = readFile("shaders/vert.spv");

        // Read the fragment shader code from the specified file.
        auto a_fragShaderCode = readFile("shaders/frag.spv");

        // Create a shader module for the vertex shader using the read shader code.
        VkShaderModule a_vertShaderModule = createShaderModule_09(a_vertShaderCode);

        // Create a shader module for the fragment shader using the read shader code.
        VkShaderModule a_fragShaderModule = createShaderModule_09(a_fragShaderCode);

        // Define the vertex shader stage information for the graphics pipeline.
        VkPipelineShaderStageCreateInfo a_vertShaderStageInfo{};
        a_vertShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        a_vertShaderStageInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
        a_vertShaderStageInfo.module = a_vertShaderModule;
        a_vertShaderStageInfo.pName = "main";

        // Define the fragment shader stage information for the graphics pipeline.
        VkPipelineShaderStageCreateInfo a_fragShaderStageInfo{};
        a_fragShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        a_fragShaderStageInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
        a_fragShaderStageInfo.module = a_fragShaderModule;
        a_fragShaderStageInfo.pName = "main";

        // Array of VkPipelineShaderStageCreateInfo structures for the shader stages in the graphics pipeline.
        // This array includes the vertex and fragment shader stages, which are necessary for processing vertex data
        // and producing the final rendered image.
        ...
        // This array includes the vertex and fragment shader stages, which are necessary for processing vertex data
        // and producing the final rendered image.
        VkPipelineShaderStageCreateInfo shaderStages[] = {a_vertShaderStageInfo, a_fragShaderStageInfo};

        // Destroy the fragment shader module to free up resources.
        // This is done after the shader stages have been created and are no longer needed.
        vkDestroyShaderModule(gbl_04_device, a_fragShaderModule, nullptr);

        // Destroy the vertex shader module to free up resources.
        // This is done after the shader stages have been created and are no longer needed.
        vkDestroyShaderModule(gbl_04_device, a_vertShaderModule, nullptr);
    }
