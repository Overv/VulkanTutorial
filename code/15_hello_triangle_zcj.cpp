...
private:
    ...
    VkCommandBuffer gbl_14_commandBuffer;

    VkSemaphore gbl_15_imageAvailableSemaphore; //cj-15
    VkSemaphore gbl_15_renderFinishedSemaphore; //cj-15
    VkFence gbl_15_inFlightFence;  //cj-15
    ...
    void initVulkan() {
        ...
        createCommandBuffer_14();
        createSyncObjects_15();   //cj-15
    }
    ...
    void mainLoop() {
         ...
         *
         * This function blocks until the device has completed all its operations.
         * It is typically used during cleanup to ensure that no operations are pending
         * before resources are destroyed.
         *
         * @param gbl_04_device The Vulkan device to wait on.
         */
        vkDeviceWaitIdle(gbl_04_device); //cj-15
    }

    void cleanup_00() {
        vkDestroySemaphore(gbl_04_device, gbl_15_renderFinishedSemaphore, nullptr); //cj-15
        vkDestroySemaphore(gbl_04_device, gbl_15_imageAvailableSemaphore, nullptr); //cj-15
        vkDestroyFence(gbl_04_device, gbl_15_inFlightFence, nullptr);  //cj-15
    ...
    void createRenderPass_11() {
        ...
        //cj-15 Define a subpass dependency to synchronize the render pass stages
        VkSubpassDependency dependency{};

        //15 Specify the source subpass index
        dependency.srcSubpass = VK_SUBPASS_EXTERNAL;

        //cj-15 Specify the destination subpass index
        dependency.dstSubpass = 0;

        //15 Specify the pipeline stage at which the source subpass will wait
        dependency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;

        //cj-15 Specify the access mask for the source subpass
        dependency.srcAccessMask = 0;

        //cj-15 Specify the pipeline stage at which the destination subpass will wait
        dependency.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;

        //cj-15 Specify the access mask for the destination subpass
        ...
        renderPassInfo.dependencyCount = 1;  //cj-15
        renderPassInfo.pDependencies = &dependency; //cj-15
    ...
    void createCommandBuffer_14() {
        ...
        if (vkAllocateCommandBuffers(gbl_04_device, &allocInfo, &gbl_14_commandBuffer) != VK_SUCCESS) {
    ...
    void recordCommandBuffer_14(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        viewport.width = static_cast<float>(gbl_06_swapChainExtent.width);
        viewport.height = static_cast<float>(gbl_06_swapChainExtent.height);
    ...
    /**
 * @brief Creates synchronization objects for a frame.
 *
 * This function creates semaphores and a fence used for synchronizing the rendering process.
 * It initializes the semaphore and fence creation structures, and then creates the synchronization
 * objects. If any of the creation calls fail, an exception is thrown.
 */
void createSyncObjects_15() {
    // Structure specifying semaphore creation info
    VkSemaphoreCreateInfo semaphoreInfo{};
    semaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;

    // Structure specifying fence creation info
    VkFenceCreateInfo fenceInfo{};
    fenceInfo.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
    fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT;

    // Create semaphores and fence, throw an exception if any creation fails
    if (vkCreateSemaphore(gbl_04_device, &semaphoreInfo, nullptr, &gbl_15_imageAvailableSemaphore) != VK_SUCCESS ||
        vkCreateSemaphore(gbl_04_device, &semaphoreInfo, nullptr, &gbl_15_renderFinishedSemaphore) != VK_SUCCESS ||
        vkCreateFence(gbl_04_device, &fenceInfo, nullptr, &gbl_15_inFlightFence) != VK_SUCCESS) {
        throw std::runtime_error("failed to create synchronization objects for a frame!");
    }
}

    /**
 * @brief Draws a frame by submitting command buffers and presenting the image.
 *
 * This function handles the synchronization and submission of command buffers
 * to the graphics queue, and presents the rendered image to the swap chain.
 * It waits for the previous frame to finish, acquires the next image from the
 * swap chain, records the command buffer, and submits it to the graphics queue.
 * Finally, it presents the image to the swap chain.
 */
    void drawFrame_15() {
        // Wait for the previous frame to finish
        vkWaitForFences(gbl_04_device, 1, &gbl_15_inFlightFence, VK_TRUE, UINT64_MAX);
        vkResetFences(gbl_04_device, 1, &gbl_15_inFlightFence);

        // Acquire the next image from the swap chain
        uint32_t imageIndex;
        vkAcquireNextImageKHR(gbl_04_device, gbl_06_swapChain, UINT64_MAX, gbl_15_imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);

        // Reset the command buffer and record commands
        vkResetCommandBuffer(gbl_14_commandBuffer, /*VkCommandBufferResetFlagBits*/ 0);
        recordCommandBuffer_14(gbl_14_commandBuffer, imageIndex);

        // Prepare the submit info structure
        VkSubmitInfo submitInfo{};
        submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;

        // Specify the semaphores to wait on before executing the command buffer
        VkSemaphore waitSemaphores[] = {gbl_15_imageAvailableSemaphore};
        ...
        submitInfo.pCommandBuffers = &gbl_14_commandBuffer;

        // Specify the semaphores to signal once the command buffer has finished executing
        VkSemaphore signalSemaphores[] = {gbl_15_renderFinishedSemaphore};
        submitInfo.signalSemaphoreCount = 1;
        submitInfo.pSignalSemaphores = signalSemaphores;

        // Submit the command buffer to the graphics queue
        if (vkQueueSubmit(gbl_04_graphicsQueue, 1, &submitInfo, gbl_15_inFlightFence) != VK_SUCCESS) {
            throw std::runtime_error("failed to submit draw command buffer!");
        }

        // Prepare the present info structure
        VkPresentInfoKHR presentInfo{};
        presentInfo.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;

        // Specify the semaphores to wait on before presenting the image
        ...
        vkQueuePresentKHR(gbl_05_presentQueue, &presentInfo);
    }
