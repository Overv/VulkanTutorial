...
struct QueueFamilyIndices_03 {
    ...
    bool isComplete() {
        return graphicsAndComputeFamily.has_value() && presentFamily_05.has_value();
    }
};
...
private:
    ...
    void cleanup() {
        ...
        if (enableValidationLayers) {
            DestroyDebugUtilsMessengerEXT_02(instance, debugMessenger, nullptr);
        }
    ...
    void setupDebugMessenger() {
        ...
        if (CreateDebugUtilsMessengerEXT_02(instance, &createInfo, nullptr, &debugMessenger) != VK_SUCCESS) {
    ...
    void createLogicalDevice() {
        QueueFamilyIndices_03 indices = findQueueFamilies(physicalDevice);

        std::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
        std::set<uint32_t> uniqueQueueFamilies = {indices.graphicsAndComputeFamily.value(), indices.presentFamily_05.value()};
        ...
        vkGetDeviceQueue(device, indices.presentFamily_05.value(), 0, &presentQueue);
    }

    void createSwapChain() {
        SwapChainSupportDetails_06 swapChainSupport = querySwapChainSupport(physicalDevice);
        ...
        QueueFamilyIndices_03 indices = findQueueFamilies(physicalDevice);
        uint32_t queueFamilyIndices[] = {indices.graphicsAndComputeFamily.value(), indices.presentFamily_05.value()};

        if (indices.graphicsAndComputeFamily != indices.presentFamily_05) {
    ...
    void createCommandPool() {
        QueueFamilyIndices_03 queueFamilyIndices = findQueueFamilies(physicalDevice);
    ...
    bool isDeviceSuitable(VkPhysicalDevice device) {
        QueueFamilyIndices_03 indices = findQueueFamilies(device);
    ...
    QueueFamilyIndices_03 findQueueFamilies(VkPhysicalDevice device) {
